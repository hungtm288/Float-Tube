...
import com.floatvideo.popup.MainActivity;
import com.floatvideo.popup.extractor.exceptions.ParsingException;
import com.floatvideo.popup.extractor.exceptions.ReCaptchaException;
import com.floatvideo.popup.extractor.services.youtube.YoutubeStreamExtractor;
    ...
    protected void handleException(final Exception exception, int serviceId, final String url) {
        ...
        if (exception instanceof ReCaptchaException) {
            ...
            getHandler().post(new Runnable() {
                @Override
                public void run() {
                    callback.onError(com.floatvideo.popup.R.string.network_error);
                }
            });
            ...
            getHandler().post(new Runnable() {
                @Override
                public void run() {
                    callback.onContentErrorWithMessage(com.floatvideo.popup.R.string.live_streams_not_supported);
                }
            });
            ...
            ErrorActivity.reportError(getHandler(), getContext(), exception, MainActivity.class, null, ErrorActivity.ErrorInfo.make(ErrorActivity.REQUESTED_STREAM, getServiceName(), url, com.floatvideo.popup.R.string.youtube_signature_decryption_error));
            getHandler().post(new Runnable() {
                @Override
                public void run() {
            ...
            if (!streamInfo.errors.isEmpty()) {
                ...
                ErrorActivity.reportError(getHandler(), getContext(), exception, MainActivity.class, null, ErrorActivity.ErrorInfo.make(ErrorActivity.REQUESTED_STREAM, getServiceName(), url, com.floatvideo.popup.R.string.could_not_get_stream));
            } else {
                ErrorActivity.reportError(getHandler(), getContext(), streamInfo.errors, MainActivity.class, null, ErrorActivity.ErrorInfo.make(ErrorActivity.REQUESTED_STREAM, getServiceName(), url, com.floatvideo.popup.R.string.could_not_get_stream));
            }
            ...
            ErrorActivity.reportError(getHandler(), getContext(), exception, MainActivity.class, null, ErrorActivity.ErrorInfo.make(ErrorActivity.REQUESTED_STREAM, getServiceName(), url, com.floatvideo.popup.R.string.parsing_error));
            getHandler().post(new Runnable() {
                @Override
                public void run() {
                    callback.onUnrecoverableError(exception);
            ...
            ErrorActivity.reportError(getHandler(), getContext(), exception, MainActivity.class, null, ErrorActivity.ErrorInfo.make(ErrorActivity.REQUESTED_STREAM, getServiceName(), url, com.floatvideo.popup.R.string.general_error));
            getHandler().post(new Runnable() {
                @Override
                public void run() {
                    callback.onUnrecoverableError(exception);
